...
impl<'a, DI, MODEL> Builder<'a, DI, MODEL>
where
    DI: WriteOnlyDataCommand,
    MODEL: Model,
{
    ...
    ///
    pub(crate) fn new(di: &'a mut DI, model: MODEL, options: ModelOptions) -> Self {
    ...
    /// `default_options`
    ///
    pub fn with_model(di: &'a mut DI, model: MODEL) -> Self {
        Self {
            di,
            model,

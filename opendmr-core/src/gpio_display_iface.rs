...
macro_rules! generic_bus {
    ($GenericxBitBus:ident { type Word = $Word:ident; Pins {$($PX:ident => $x:tt,)*}}) => {
        ...
        pub struct $GenericxBitBus<'a, $($PX, )*> {
            pins: ($(&'a mut $PX, )*),
            last: $Word,
        }

        impl<'a, $($PX, )*> $GenericxBitBus<'a, $($PX, )*>
        where
            $($PX: OutputPin, )*
        {
            ...
            pub fn new(pins: ($(&'a mut $PX, )*)) -> Result<Self> {
        ...
        impl<'a, $($PX, )*> core::convert::TryFrom<($(&'a mut $PX, )*)>
            for $GenericxBitBus<'a, $($PX, )*>
        where
            $($PX: OutputPin, )*
        {
            ...
            fn try_from(pins: ($(&'a mut $PX, )*)) -> Result<Self> {
                Self::new(pins)
            }
        }
    };
...
/// low to high edge.
pub struct PGPIO8BitInterface<'a, BUS, DC, WR> {
    bus: &'a mut BUS,
    dc: &'a mut DC,
    wr: &'a mut WR,
}

impl<'a, BUS, DC, WR> PGPIO8BitInterface<'a, BUS, DC, WR>
where
...
impl<'a, BUS, DC, WR> WriteOnlyDataCommand for PGPIO8BitInterface<'a, BUS, DC, WR>
where
    BUS: OutputBus<Word = u8>,
    DC: OutputPin,
    WR: OutputPin,
